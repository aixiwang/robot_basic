...
class XYZ:
    __default_speed__ = 10  # mm per second
...
def xyz_init(port='COM3', baud=9600, limits=[700, 500, 100]):
    xyz = XYZ(limits)
    xyz.startup(port, baud)
    return xyz
    
if __name__ == "__main__":
    motion = xyz_init(port='/dev/ttyUSB1',baud=9600,limits=[700, 500, 100])
    motion.moveto(dx=10,dy=10,dz=10,speed=10)
    motion.moverel(dx=10,dy=10,dz=10,speed=10)
