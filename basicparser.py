...
class BASICParser:
    ...
    def __evaluate_function(self, category):
        ...
        if category == Token.RND:
            ...
            try:
                # TODO
                self.motion = xyz_init(port='/dev/ttyUSB1',baud=115200,limits=[700, 500, 100])
                return 0 

            except ValueError:
                print(ValueError("Invalid value supplied to MINIT in line " +
                                 str(self.__line_number)))
                self.motion = None
                return -1
                                 

        elif category == Token.MOVETO:
            try:
                ...
                # TODO
                if self.motion != None:
                    self.motion.moveto(dx=int(p1),dy=int(p2),dz=int(p3),speed=int(p4))
                    return 0
                else:
                    return -1
                    
            except ValueError:
                print(ValueError("Invalid value supplied to MOVETO in line " +
                                 str(self.__line_number)))
                return -1
            ...
            try:
                ...
                if self.motion != None:
                    self.motion.moverel(dx=int(p1),dy=int(p2),dz=int(p3),speed=int(p4))
                    return 0
                else:
                    return -1
